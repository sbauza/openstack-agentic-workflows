...
- **Verify reachability before flagging bugs**: See `rules.md` — before reporting a potential runtime failure, you MUST trace class/function instantiation to callers, check config option definitions and docs, and confirm the scenario is reachable under valid configuration. A code path that requires operator misconfiguration to trigger is not a bug in the patch.
